...
struct SwiftScript: ParsableCommand {
  ...
  struct DownloadSchema: ParsableCommand {
    ...
    func run() throws {
      ...
      let endpoint = URL(string: "Please contact project owner for this part of url :)/.netlify/functions/graphql")!
  ...
  struct GenerateCode: ParsableCommand {
    ...
    func run() throws {
      ...
      let generatedSwiftUI = targetRootURL.apollo.childFolderURL(folderName: "Generated")
      ...
      let schemaURL = try targetRootURL
        .apollo.childFolderURL(folderName: "Apollo")
        .apollo.childFileURL(fileName: "schema.graphqls")
      let codegenOptions = ApolloCodegenOptions(outputFormat: .multipleFiles(inFolderAtURL: generatedSwiftUI),
